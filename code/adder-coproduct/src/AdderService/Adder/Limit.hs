...
findLimit :: (MonadFree (Sum f) m, All Functor f, Contains AddF f, Contains ClearF f, Contains TotalF f) => m Int
findLimit = do
...
findLimit' :: (MonadFree (Sum f) m, All Functor f, Contains AddF f) => StateT Int m ()
findLimit' = do
  r <- lift $ add 1
  when r $ do
    modify (+ 1)
